...
process=processes_v6_mmmt.yaml
...
fo=2016_${mainout}_mmmt
cat=cat_v6_mmmt_2016.yaml
...
#python MakeDistributions_v6.py -c $cat -csv $csv -i $input -p $process -o $output1 -ch mmmt -s -ddZH -fi $fo

echo "Making Plots ..."
python MakePlots_skimmed.py -i skimmed_${output1}.root -o $output1 -c $cat --ch mmmt -ddZH -p $process
#
#python MakePlots_skimmed_cats.py -i ${output1} -o ${output1}_cats -c $cat --ch mmmt -p $process -r 
#python MakePlots_skimmed_sys.py -i skimmed_${output1}.root -o $output1 -c $cat --ch mmmt -p $process
