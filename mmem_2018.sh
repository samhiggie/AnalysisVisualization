...
python MakeDistributions_v7.py -c $catsys -csv $csv -i $input -p $process -o $output1 -ch ${channel} -s -fi $fo -fo $output1 -year $year

hadd -f skimmed_${output1}_combined.root skimmed_${output1}.root skimmed_${output1}_sys.root

python MakePlots_skimmed_sys.py -i skimmed_${output1}_combined.root -o $output1 -c $catsys --ch ${channel} -p $process -de -year $year
...
python MakeDistributions_v7.py -c $catsys -csv $csv -i $input -p $process -o $output1ex -ch ${channel} -s -fi $fo -fo $output1ex -year $year -ex

hadd -f skimmed_${output1ex}_combined.root skimmed_${output1ex}.root skimmed_${output1ex}_sys.root

python MakePlots_skimmed_sys.py -i skimmed_${output1ex}_combined.root -o $output1ex -c $catsys --ch ${channel} -p $process -de -year $year
