...
from MakeDistributions_v7 import *
...
def getPlotStuffs(year,channel):
    ...
    return lumi,cms,pre
def createFakeFactorHistoslocal(allcats, inputFFile,prompt):
    newVarVals={}
    if prompt:
    ...
    for cat in histodict.keys():
        ...
        for treename in treetypes:
            ...
            if "prompt" in treename: 
                mask = fakefactorArray["gen_match_3"] == 15
                #print fakefactorArray["gen_match_3"]
                #print mask
                for var,branch in fakefactorArray.iteritems():
...
if __name__ == "__main__":
    ...
    parser.add_argument("-pt",  "--prompt", default=False,action='store_true',  help="When making fake rate histograms, subtract the prompt contribution")
    ...
    with uproot.open("skimmed_"+str(args.ffin)+".root") as inputFFile:
        for cat in inputFFile.keys():
            infofile.write(cat)
            infofile.write("\n")
        histodict = createFakeFactorHistoslocal(allcats, inputFFile,args.prompt)
    ...
    if args.ftype=="firstleg" or args.ftype=="secondleg":
        ...
        for cat in measurecats:
            ...
            for i,cut in allcats[cat].cuts.iteritems():
                infofile.write(i)
                infofile.write(" : ")
                for subcut in cut:
