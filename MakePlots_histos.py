    ...
    #for ivar,var in enumerate(cat.vars.keys()):
        ...
        for categoryKey in file.GetListOfKeys():
            ...
            for histokey in category.GetListOfKeys():
                ...
                if  histoname.split("fake1_")[0]!="" and histoname.split("fake2_")[0]!="":
            ...
            hMCList = hMCDict.keys()
            hMCList.sort()

            for histo in hMCList:
                if histo in Bkg:
            ...
            if args.mhs:
                if var in ["mll_m40","mll","mll_fine","mll_fit0","mll_fit1","mll_fit2","mll_fit3"]:
                    histoout = ROOT.TFile.Open("final_"+str(category.GetName())+"_"+str(var)+".root","recreate")
                    histoout.mkdir(str(category.GetName()))
                    histoout.cd(str(category.GetName()))
                    if args.mc:
